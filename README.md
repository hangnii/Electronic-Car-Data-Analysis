...
- 지역별 전기차 현황과 지역별 대기환경 및 주관적 환경 인식이 상관관계가 있지 않을까 하는 가설을 세우고 데이터를 수집하여, 각 데이터의 양상 및 상관관계 분석을 수행한 EDA 프로젝트. 
-  mysql을 사용해 데이터 전처리를 하고 python 판다스와 시각화 라이브러리를 활용해 각 변수와 전기차 현황과의 관계를 분석한 프로젝트.
</br>
</br>
</br>
